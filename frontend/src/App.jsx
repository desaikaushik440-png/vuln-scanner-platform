...
function AuthPage({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#070b14", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI',sans-serif", padding: "20px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: "20%", left: "50%", transform: "translateX(-50%)", width: "600px", height: "300px", background: "radial-gradient(ellipse,rgba(99,102,241,0.15) 0%,transparent 70%)" }} />
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: "420px" }}>
        <div style={{ textAlign: "center", marginBottom: "40px" }}>
          ...
          <h1 style={{ color: "white", fontSize: "28px", fontWeight: "800", margin: "0 0 6px" }}>VulnScanner</h1>
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "20px", padding: "32px", backdropFilter: "blur(20px)", boxShadow: "0 25px 60px rgba(0,0,0,0.5)" }}>
          ...
          {error && <div style={{ padding: "12px 16px", background: error.includes("created") ? "rgba(46,213,115,0.1)" : "rgba(255,71,87,0.1)", border: "1px solid " + (error.includes("created") ? "rgba(46,213,115,0.3)" : "rgba(255,71,87,0.3)"), borderRadius: "10px", color: error.includes("created") ? "#2ed573" : "#ff4757", fontSize: "13px", marginBottom: "16px" }}>{error}</div>}
          <button onClick={handleSubmit} disabled={loading}
            style={{ width: "100%", padding: "14px", background: loading ? "rgba(99,102,241,0.3)" : "linear-gradient(135deg,#6366f1,#8b5cf6)", border: "none", borderRadius: "12px", color: "white", fontSize: "15px", fontWeight: "700", cursor: loading ? "not-allowed" : "pointer", boxShadow: loading ? "none" : "0 8px 25px rgba(99,102,241,0.4)" }}>
      ...
      <div style={{ position: "relative", display: "inline-block", marginBottom: "12px" }}>
        <svg width="110" height="110" viewBox="0 0 110 110">
          ...
          <circle cx="55" cy="55" r="45" fill="none" stroke={color} strokeWidth="8"
            strokeDasharray={`${(score/100)*c} ${c}`} strokeLinecap="round" transform="rotate(-90 55 55)"
...
function AdminPage({ token }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(API + "/api/admin/scans", { headers }).then(r => r.json()),
    ]).then(([s, u, sc]) => {
      if (s.success) setStats(s);
      if (u.success) setUsers(u.users);
      if (sc.success) setScans(sc.scans);
      setLoading(false);
    });
  }, []);
  ...
  const statCards = stats ? [
    ...
    { label: "Total Scans",   value: stats.totalScans,  color: "#2ed573", icon: "🔍" },
    { label: "Scans Today",   value: stats.scansToday,  color: "#ffd32a", icon: "📅" },
  ] : [];

  return (
    <div style={{ flex: 1, padding: "clamp(20px,3vw,48px)", maxWidth: "980px", margin: "0 auto", width: "100%" }}>
      <div style={{ marginBottom: "32px" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,71,87,0.1)", border: "1px solid rgba(255,71,87,0.2)", borderRadius: "20px", padding: "6px 14px", fontSize: "12px", color: "#ff4757", fontWeight: "600", marginBottom: "16px" }}>
          🔐 ADMIN PANEL
        </div>
        <h1 style={{ fontSize: "clamp(24px,4vw,36px)", fontWeight: "900", margin: "0 0 8px", letterSpacing: "-1px" }}>Admin Dashboard</h1>
        <p style={{ color: "rgba(255,255,255,0.35)", margin: 0 }}>Monitor all users and scan activity</p>
      </div>

      {/* TABS */}
      <div style={{ display: "flex", gap: "8px", marginBottom: "28px", background: "rgba(255,255,255,0.03)", padding: "4px", borderRadius: "14px", width: "fit-content", border: "1px solid rgba(255,255,255,0.06)" }}>
        {[{ id: "overview", label: "📊 Overview" }, { id: "users", label: "👥 Users" }, { id: "scans", label: "🔍 All Scans" }].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ padding: "10px 20px", background: tab===t.id ? "linear-gradient(135deg,#6366f1,#8b5cf6)" : "transparent", border: "none", borderRadius: "10px", color: tab===t.id ? "white" : "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: "13px", fontWeight: "600", transition: "all 0.2s", whiteSpace: "nowrap" }}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          {tab === "overview" && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: "16px", marginBottom: "32px" }}>
                {statCards.map((s, i) => (
                  <div key={i} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "20px", padding: "24px", textAlign: "center" }}>
                    <div style={{ fontSize: "32px", marginBottom: "12px" }}>{s.icon}</div>
                    <div style={{ fontSize: "36px", fontWeight: "900", color: s.color, marginBottom: "6px" }}>{s.value}</div>
                    <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.3)", letterSpacing: "1px" }}>{s.label.toUpperCase()}</div>
                  </div>
                ))}
              </div>

              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "20px", padding: "24px" }}>
                <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)", letterSpacing: "2px", marginBottom: "16px" }}>RECENT ACTIVITY</div>
                {scans.slice(0, 5).map((s, i) => {
                  const rs = parseInt(s.risk_score) || 0;
                  const rc = rs >= 70 ? "#ff4757" : rs >= 45 ? "#ff6b35" : rs >= 20 ? "#ffd32a" : "#2ed573";
                  return (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: i < 4 ? "1px solid rgba(255,255,255,0.04)" : "none", flexWrap: "wrap", gap: "8px" }}>
                      <div>
                        <div style={{ fontSize: "14px", fontWeight: "600", color: "#818cf8", fontFamily: "monospace" }}>{s.target}</div>
                        <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)", marginTop: "2px" }}>by {s.user_name} · {new Date(s.created_at).toLocaleString()}</div>
                      </div>
                      <span style={{ color: rc, fontWeight: "800", fontFamily: "monospace" }}>{rs}/100</span>
                    </div>
                  );
                })}
              </div>
                      ...
                      <tr key={u.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                        <td style={{ padding: "16px 20px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                            <div style={{ width: "32px", height: "32px", borderRadius: "8px", background: "linear-gradient(135deg,#6366f1,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", fontWeight: "800", flexShrink: 0 }}>{u.name[0].toUpperCase()}</div>
                            <span style={{ fontWeight: "600", color: "white" }}>{u.name}</span>
                          </div>
                        </td>
                        <td style={{ padding: "16px 20px", color: "rgba(255,255,255,0.4)", fontSize: "12px" }}>{u.email}</td>
                        <td style={{ padding: "16px 20px" }}>
                          <span style={{ padding: "3px 10px", borderRadius: "20px", fontSize: "11px", fontWeight: "700", background: u.role==="ADMIN" ? "rgba(255,71,87,0.1)" : "rgba(99,102,241,0.1)", color: u.role==="ADMIN" ? "#ff4757" : "#818cf8", border: "1px solid " + (u.role==="ADMIN" ? "rgba(255,71,87,0.2)" : "rgba(99,102,241,0.2)") }}>
                            {u.role}
                          </span>
                        </td>
                        <td style={{ padding: "16px 20px", color: "#2ed573", fontWeight: "700" }}>{u.scan_count}</td>
                        <td style={{ padding: "16px 20px", color: "rgba(255,255,255,0.3)", fontSize: "12px" }}>{new Date(u.created_at).toLocaleDateString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                    ...
                    {scans.map((s, i) => {
                      ...
                      return (
                        <tr key={s.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                          ...
                          <td style={{ padding: "16px 20px" }}>
                            <div style={{ fontSize: "13px", color: "white", fontWeight: "600" }}>{s.user_name}</div>
                            <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)" }}>{s.user_email}</div>
                          </td>
                          <td style={{ padding: "16px 20px" }}>
                          ...
                          <td style={{ padding: "16px 20px", color: "rgba(255,255,255,0.3)", fontSize: "12px" }}>{new Date(s.created_at).toLocaleString()}</td>
                        </tr>
                      );
                    })}
                  </tbody>
...
function ScannerPage({ token, user, onLogout }) {
  ...
  const navItems = [
    ...
    ...(isAdmin ? [{ id: "admin", icon: "🔐", label: "Admin Panel", admin: true }] : []),
  ];
    ...
    <div style={{ minHeight: "100vh", background: "#070b14", color: "white", fontFamily: "'Segoe UI',sans-serif", display: "flex", flexDirection: "column", position: "relative" }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(7,11,20,0.8)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "0 24px", height: "60px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <button onClick={() => setMenuOpen(!menuOpen)} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.5)", cursor: "pointer", fontSize: "18px", padding: "4px" }}>☰</button>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <span style={{ fontWeight: "800", fontSize: "16px" }}>VulnScanner</span>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          {isAdmin && <div style={{ padding: "4px 12px", background: "rgba(255,71,87,0.1)", border: "1px solid rgba(255,71,87,0.2)", borderRadius: "20px", fontSize: "11px", color: "#ff4757", fontWeight: "700" }}>ADMIN</div>}
          <div style={{ padding: "6px 14px", background: "rgba(46,213,115,0.1)", border: "1px solid rgba(46,213,115,0.2)", borderRadius: "20px", fontSize: "11px", color: "#2ed573", fontWeight: "600" }}>● LIVE</div>
          <div style={{ width: "34px", height: "34px", borderRadius: "10px", background: "linear-gradient(135deg,#6366f1,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", fontWeight: "800", cursor: "pointer" }} onClick={() => setMenuOpen(!menuOpen)}>
        ...
        <div style={{ position: "fixed", top: "60px", left: 0, bottom: 0, width: "240px", background: "rgba(7,11,20,0.95)", backdropFilter: "blur(20px)", borderRight: "1px solid rgba(255,255,255,0.06)", padding: "24px 12px", display: "flex", flexDirection: "column", zIndex: 99, transform: menuOpen ? "translateX(0)" : "translateX(-100%)", transition: "transform 0.25s ease" }}>
          ...
          {navItems.map(n => (
            <button key={n.id} onClick={() => { setPage(n.id); setMenuOpen(false); }}
              style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px 14px", background: page===n.id ? (n.admin ? "rgba(255,71,87,0.1)" : "rgba(99,102,241,0.15)") : "transparent", color: page===n.id ? (n.admin ? "#ff4757" : "#818cf8") : "rgba(255,255,255,0.4)", border: "none", borderRadius: "12px", cursor: "pointer", textAlign: "left", fontSize: "14px", fontWeight: page===n.id ? "700" : "400", marginBottom: "4px", transition: "all 0.2s", borderLeft: page===n.id ? "3px solid " + (n.admin ? "#ff4757" : "#6366f1") : "3px solid transparent" }}>
          ...
          <div style={{ padding: "16px", background: "rgba(255,255,255,0.02)", borderRadius: "14px", border: "1px solid rgba(255,255,255,0.05)" }}>
            ...
            <button onClick={onLogout} style={{ width: "100%", padding: "9px", background: "rgba(255,71,87,0.1)", border: "1px solid rgba(255,71,87,0.2)", borderRadius: "10px", color: "#ff4757", cursor: "pointer", fontSize: "13px", fontWeight: "600" }}>Sign Out</button>
          </div>
        </div>
            ...
            {page === "scan" && (
              <div>
                <div style={{ marginBottom: "36px" }}>
                  <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", borderRadius: "20px", padding: "6px 14px", fontSize: "12px", color: "#818cf8", fontWeight: "600", marginBottom: "16px" }}>⚡ VULNERABILITY SCANNER</div>
                  <h1 style={{ fontSize: "clamp(24px,4vw,36px)", fontWeight: "900", margin: "0 0 10px", letterSpacing: "-1px", lineHeight: 1.1 }}>
                    Scan for{" "}<span style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6,#ec4899)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Vulnerabilities</span>
                  </h1>
                ...
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "20px", padding: "24px", marginBottom: "24px" }}>
                  <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                    <div style={{ flex: 1, minWidth: "200px", position: "relative" }}>
                      ...
                      <input type="text" placeholder="scanme.nmap.org or 192.168.1.1" value={target}
                        onChange={e => setTarget(e.target.value)} onKeyDown={e => e.key === "Enter" && handleScan()}
                        style={{ width: "100%", padding: "16px 16px 16px 36px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "14px", color: "white", fontSize: "15px", outline: "none", boxSizing: "border-box", fontFamily: "monospace" }} />
                    </div>
                    <button onClick={handleScan} disabled={loading}
                      style={{ padding: "16px 28px", background: loading ? "rgba(99,102,241,0.2)" : "linear-gradient(135deg,#6366f1,#8b5cf6)", border: "none", borderRadius: "14px", color: "white", cursor: loading ? "not-allowed" : "pointer", fontSize: "15px", fontWeight: "700", whiteSpace: "nowrap", boxShadow: loading ? "none" : "0 8px 25px rgba(99,102,241,0.4)" }}>
                  ...
                  {scanLog.length > 0 && (
                    <div style={{ marginTop: "20px", background: "rgba(0,0,0,0.3)", borderRadius: "12px", padding: "16px", fontFamily: "monospace", fontSize: "12px", border: "1px solid rgba(255,255,255,0.04)" }}>
                      {scanLog.map((log, i) => (
                        <div key={i} style={{ color: log.includes("✓") ? "#2ed573" : "rgba(255,255,255,0.4)", marginBottom: "5px" }}>
                          <span style={{ color: log.includes("✓") ? "#2ed573" : "#6366f1", marginRight: "8px" }}>{log.includes("✓") ? "✓" : "▸"}</span>{log}
                        </div>
                      ))}
                ...
                {error && <div style={{ padding: "16px", background: "rgba(255,71,87,0.08)", border: "1px solid rgba(255,71,87,0.2)", borderRadius: "14px", color: "#ff4757", marginBottom: "24px" }}>⚠ {error}</div>}
                ...
                {result && (
                  <div>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px", flexWrap: "wrap", gap: "12px" }}>
                      ...
                      {scanId && <button onClick={() => downloadReport(scanId)} style={{ padding: "12px 24px", background: "rgba(46,213,115,0.1)", border: "1px solid rgba(46,213,115,0.25)", borderRadius: "12px", color: "#2ed573", cursor: "pointer", fontSize: "14px", fontWeight: "700" }}>📄 Export Report</button>}
                    </div>
                    ...
                    {result.vulnerabilities && result.vulnerabilities.length > 0 && (
                      <div style={{ marginBottom: "28px" }}>
                        <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)", letterSpacing: "2px", marginBottom: "14px", display: "flex", alignItems: "center", gap: "8px" }}>
                          <div style={{ flex: 1, height: "1px", background: "rgba(255,255,255,0.06)" }} />THREAT ASSESSMENT<div style={{ flex: 1, height: "1px", background: "rgba(255,255,255,0.06)" }} />
                        </div>
                            ...
                            return (
                              <div key={i} style={{ background: s.bg, border: "1px solid "+s.border, borderRadius: "16px", padding: "18px", position: "relative", overflow: "hidden" }}>
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px", gap: "8px" }}>
                                  ...
                                  <span style={{ fontSize: "10px", color: s.color, border: "1px solid "+s.border, padding: "3px 10px", borderRadius: "20px", whiteSpace: "nowrap", fontWeight: "700" }}>{v.severity}</span>
                                </div>
                                {v.cve && <div style={{ fontSize: "11px", color: "#ff6b35", marginBottom: "8px", fontFamily: "monospace" }}>🔖 {v.cve}{v.cvssScore ? " · CVSS "+v.cvssScore : ""}</div>}
                    ...
                    <div style={{ marginBottom: "28px" }}>
                      <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)", letterSpacing: "2px", marginBottom: "14px", display: "flex", alignItems: "center", gap: "8px" }}>
                        <div style={{ flex: 1, height: "1px", background: "rgba(255,255,255,0.06)" }} />PORT SCAN RESULTS<div style={{ flex: 1, height: "1px", background: "rgba(255,255,255,0.06)" }} />
                      </div>
                      {(!result.ports || result.ports.length === 0) ? (
                        <div style={{ textAlign: "center", padding: "40px", color: "rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.02)", borderRadius: "16px" }}>No open ports detected</div>
                      ) : (
                              ...
                              <tbody>
                                {result.ports.map((p, i) => (
                                  <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                                    ...
                                    <td style={{ padding: "14px 18px" }}><span style={{ color: "#2ed573", background: "rgba(46,213,115,0.1)", padding: "4px 12px", borderRadius: "20px", fontSize: "11px", fontWeight: "700", border: "1px solid rgba(46,213,115,0.2)" }}>● {p.state.toUpperCase()}</span></td>
                    ...
                    <div>
                      <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)", letterSpacing: "2px", marginBottom: "14px", display: "flex", alignItems: "center", gap: "8px" }}>
                        <div style={{ flex: 1, height: "1px", background: "rgba(255,255,255,0.06)" }} />RAW NMAP OUTPUT<div style={{ flex: 1, height: "1px", background: "rgba(255,255,255,0.06)" }} />
                      </div>
                      <pre style={{ background: "rgba(0,0,0,0.4)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px", padding: "20px", overflow: "auto", color: "#2ed573", fontSize: "12px", lineHeight: "1.9", margin: 0, fontFamily: "monospace" }}>{result.scan_result}</pre>
                    </div>
                  </div>
                )}
              </div>
            ...
            {page === "history" && (
              <div>
                <div style={{ marginBottom: "36px" }}>
                  ...
                  <h1 style={{ fontSize: "clamp(24px,4vw,36px)", fontWeight: "900", margin: "0 0 8px", letterSpacing: "-1px" }}>Scan History</h1>
                  <p style={{ color: "rgba(255,255,255,0.35)", margin: 0 }}>All your previous vulnerability scans</p>
                </div>
                {histLoading ? <p style={{ color: "rgba(255,255,255,0.3)" }}>Loading...</p>
                  : history.length === 0 ? (
                          ...
                          <tbody>
                            {history.map((s) => {
                              ...
                              return (
                                <tr key={s.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                                  <td style={{ padding: "16px 20px", color: "#818cf8", fontWeight: "700", fontFamily: "monospace" }}>{s.target}</td>
                                  <td style={{ padding: "16px 20px" }}><span style={{ color: rc, fontWeight: "800", fontFamily: "monospace", fontSize: "16px" }}>{rs}</span><span style={{ color: "rgba(255,255,255,0.2)", fontSize: "11px" }}>/100</span></td>
                                  <td style={{ padding: "16px 20px", color: "rgba(255,255,255,0.35)", fontSize: "12px" }}>{new Date(s.createdAt).toLocaleDateString()}</td>
                                  <td style={{ padding: "16px 20px" }}><button onClick={() => downloadReport(s.id)} style={{ padding: "7px 16px", background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", borderRadius: "8px", color: "#818cf8", cursor: "pointer", fontSize: "12px", fontWeight: "700" }}>Export</button></td>
                                </tr>
                              );
                            })}
                          </tbody>
